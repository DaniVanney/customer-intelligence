...
def build_inactivity_features(data, cutoff_date, observation_days=180):
    cutoff = pd.Timestamp(cutoff_date).normalize()

    customer_features = build_customer_features(data, cutoff)

    eligible_customers = customer_features.loc[
        customer_features["recency_days"] <= observation_days
    ].copy()
    ...
    eligible_customers[RECENT_ACTIVITY_COLUMNS] = eligible_customers[
        RECENT_ACTIVITY_COLUMNS
    ].fillna(0)
    ...
    eligible_customers[integer_columns] = eligible_customers[
        integer_columns
    ].astype("int64")
...
def build_inactivity_snapshot(data, cutoff_date, observation_days=180, prediction_days=90):
    ...
    eligible_customers = build_inactivity_features(
        data,
        cutoff,
        observation_days
    )

    future_mask = (data["invoice_date"] >= cutoff) & (
        data["invoice_date"] < prediction_end
    )
    ...
    future_customers = reconciled_future.loc[
        reconciled_future["effective_quantity"] > 0,
        "customer_id"
    ].dropna().unique()

    eligible_customers.insert(2, "prediction_end_date", prediction_end)

    eligible_customers["is_inactive"] = ~eligible_customers[
        "customer_id"
